...
class Main extends Sprite
{
		...
		if(Assets.exists("data/menu.json"))
		{
			...
			if(Reflect.hasField(_dataObj,"SoundBoard"))
			{
				...
				for(i in 0 ... soundBoardList.length)
				{
					...
					//buttonHolder.drawOnResize = true;
					...
					if(Reflect.hasField(soundBoardList[i],"Button"))
					{
						...
						for( j in 0 ... buttonList.length)
						{
							...
							var button:SoundButton = new SoundButton({"Label":{"textColor":0x000000,"size":BUTTON_FONT_SIZE},"text": Reflect.field(buttonList[j],"Name"),"width":200, "height": 80} );
							...
							{
								#if !html5 
									_soundManager.load( Reflect.field(soundDataList[a],"Sound"), Std.string(FileSystem.absolutePath("") + "/sound/" + Reflect.field(soundDataList[a],"Sound") + ".ogg") );
									trace("Sound : " + Reflect.field(soundDataList[a],"Sound") + " => " + FileSystem.exists(FileSystem.absolutePath("") + "/sound/" + Reflect.field(soundDataList[a],"Sound") + ".ogg" ));
								#end
							...
							//button.loop = (Reflect.hasField(buttonList[j],"loop")) ? Reflect.field(buttonList[j],"loop") : false;
						...
						//_tabArea.getTabButton(i).buttonMode = true;
	...
	private function onSoundButtonClick(event:MouseEvent) :Void
	{
		...
		{
			var button:SoundButton = new SoundButton( {"Label":{"textColor":0x000000,"size":BUTTON_FONT_SIZE},"name":"button_" + i, "Loop": Reflect.field(buttonSoundInfoList[i],"Loop"), "SoundName": Reflect.field(buttonSoundInfoList[i],"Sound"), "Music":Reflect.hasField(buttonSoundInfoList[i],"Music"),"text":Reflect.field(buttonSoundInfoList[i],"Name"),"width": 200, "height": 80 });
